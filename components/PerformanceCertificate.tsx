    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] w-full max-w-2xl mx-auto animate-fadeIn gap-8 p-4">
        <div className="bg-white rounded-[3rem] p-12 text-center shadow-2xl border-4 border-[#27AE60]/20 w-full">
           ...
           <h2 className="text-3xl font-black text-[#2D3748] uppercase tracking-tighter italic mb-4">Keep Going!</h2>
           <div className="bg-green-50 border border-[#27AE60]/20 p-6 rounded-2xl mb-8">
             ...
             <div className="mt-4 flex flex-col items-center">
               ...
               <span className="text-5xl font-black text-[#27AE60]">{score}</span>
             </div>
           </div>
           ...
           <div className="flex flex-col gap-4">
             ...
             <button 
               ...
               className="w-full bg-white text-[#5D6D61] border-2 border-[#D6C2B0] py-5 rounded-[2rem] font-black uppercase text-xs tracking-[0.2em] hover:bg-green-50 hover:text-[#27AE60] transition-all flex items-center justify-center gap-3 active:scale-95"
             >
               <ArrowLeft size={18} />
               <span>{exitText}</span>
             </button>
    ...
    <div className="flex flex-col items-center gap-8 w-full max-w-4xl mx-auto animate-fadeIn pb-20">
      ...
      <div ref={certRef} className="w-full bg-white p-8 md:p-12 relative rounded-[2rem] shadow-2xl border-[12px] border-double border-[#27AE60] overflow-hidden text-[#1A2F1F]">
         ...
         <div className="relative z-10 flex flex-col items-center text-center space-y-6">
            ...
            <div className="space-y-2">
               <h2 className="text-3xl md:text-5xl font-black text-[#2D3748] uppercase tracking-tighter font-serif leading-tight">
                 {TITLES[type] || 'CERTIFICATE OF ACHIEVEMENT'}
               </h2>
               <p className="text-[#5D6D61] font-bold text-sm tracking-[0.4em] uppercase">TRAN HUNG DAO HIGH SCHOOL • Grade 11</p>
            </div>

            <div className="w-full h-px bg-[#27AE60]/30 my-4 max-w-lg"></div>

            <div className="space-y-1">
               <p className="text-sm font-bold text-[#5D6D61] italic font-serif">This is to certify that</p>
               {/* Ensure name is uppercase and serif for classic look */}
               <h3 className="text-4xl font-black text-[#27AE60] italic font-serif uppercase tracking-wide">{studentName}</h3>
               {studentUsername && <p className="text-[10px] font-bold text-[#5D6D61] uppercase tracking-widest mt-1">Account: {studentUsername}</p>}
            </div>

            <div className="space-y-2">
               <p className="text-sm font-bold text-[#5D6D61] italic font-serif">Has successfully completed the assessment for</p>
               <p className="text-2xl font-black text-[#2D3748] uppercase tracking-wide">{unitTitle}</p>
            </div>
            ...
            <div className="grid grid-cols-2 gap-12 mt-4 w-full max-w-3xl bg-green-50 p-8 rounded-2xl border border-[#27AE60]/20 shadow-sm backdrop-blur-sm">
               <div className="text-center flex flex-col justify-center">
                  <p className="text-[10px] font-black text-[#5D6D61] uppercase tracking-widest mb-2">Performance Score</p>
                  <p className="text-5xl md:text-7xl font-black text-[#27AE60]">{customScoreDisplay || `${score}%`}</p>
               </div>
               <div className="text-left border-l border-[#27AE60]/20 pl-8 flex flex-col justify-center">
                  <p className="text-[10px] font-black text-[#5D6D61] uppercase tracking-widest mb-2">AI Examiner Feedback</p>
                  <p className="text-sm font-medium text-[#2D3748] italic leading-relaxed">"{feedback}"</p>
               </div>
            </div>

            <div className="pt-8 flex flex-col items-center space-y-2 opacity-80">
               <div className="w-48 border-b-2 border-[#2D3748]/20"></div>
               <p className="text-[10px] font-black text-[#2D3748] uppercase tracking-widest">Teacher Vo Thi Thu Ha</p>
               <p className="text-[8px] text-[#5D6D61] uppercase tracking-widest">Academic Director</p>
            </div>
         </div>
      </div>
      ...
      <div className="flex flex-col md:flex-row gap-4 w-full max-w-2xl">
         <button
           ...
           className="flex-1 bg-white text-[#27AE60] border-2 border-[#27AE60] py-4 rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-[#27AE60] hover:text-white transition-all flex items-center justify-center gap-2 shadow-lg active:scale-95"
         >
         ...
         {isSuccess && onNextStep ? (
           <button
             ...
             className="flex-[2] bg-[#27AE60] text-white py-4 rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-[#2ECC71] transition-all flex items-center justify-center gap-2 shadow-lg active:scale-95 border-b-4 border-[#1E8449] active:border-b-0 active:translate-y-1"
           >
             <span>{nextLabel}</span>
           ...
           <button
             onClick={onSaveAndExit}
             className="flex-[2] bg-slate-800 text-white py-4 rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-black transition-all flex items-center justify-center gap-2 shadow-lg active:scale-95"
           >
             <ArrowLeft size={18} />
             <span>{exitText}</span>
           </button>
