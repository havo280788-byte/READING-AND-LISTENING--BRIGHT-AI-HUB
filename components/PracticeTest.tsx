      ...
      <div className="min-h-screen -m-4 md:-m-8 p-4 md:p-8 flex flex-col items-center justify-center animate-fadeIn">
        <div className="max-w-2xl w-full bg-white backdrop-blur-md rounded-[3rem] border-4 border-[#27AE60]/20 shadow-2xl p-12 text-center space-y-10">
          ...
          <div className="space-y-4">
            <h2 className="text-4xl font-black text-[#2D3748] uppercase italic tracking-tighter">Under Construction</h2>
            <p className="text-[#5D6D61] font-medium italic text-lg leading-relaxed">
          ...
          <button 
            onClick={onReturn}
            className="w-full bg-[#27AE60] text-white py-6 rounded-[2rem] font-black uppercase text-xs tracking-[0.4em] shadow-xl hover:bg-[#2ECC71] transition-all flex items-center justify-center space-x-3"
          >
            <ArrowLeft size={18} />
      ...
      <div className="min-h-screen -m-4 md:-m-8 p-4 md:p-8 flex flex-col items-center justify-center bg-white">
        {isPass ? (
          <div className="w-full">
            ...
            <div className="flex justify-center mt-8">
               <button 
                 ...
                 className="px-8 py-3 border-2 border-[#27AE60] text-[#27AE60] font-black uppercase rounded-2xl hover:bg-green-50 transition-all"
               >
          ...
          <div className="max-w-2xl w-full bg-white rounded-[3rem] p-12 text-center shadow-2xl border-4 border-[#27AE60]/20">
             ...
             <h2 className="text-3xl font-black text-[#2D3748] uppercase tracking-tighter italic mb-4">Cố gắng lên!</h2>
             <div className="bg-green-50 border border-[#27AE60]/20 p-6 rounded-2xl mb-8">
               ...
               <div className="mt-4 flex flex-col items-center">
                 <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Tổng Điểm Unit</span>
                 <span className="text-5xl font-black text-[#27AE60]">{totalScore}/100</span>
               </div>
             </div>
             <button 
               onClick={onReturn}
               className="w-full bg-[#27AE60] text-white py-5 rounded-[2rem] font-black uppercase text-xs tracking-[0.2em] shadow-xl hover:bg-[#2ECC71] transition-all flex items-center justify-center gap-3 active:scale-95"
             >
               <ArrowLeft size={18} />
      ...
      <div className="max-w-6xl mx-auto w-full space-y-8 flex-1 flex flex-col">
        ...
        <header className="flex justify-between items-center bg-white backdrop-blur-xl border border-green-100 p-6 rounded-[2.5rem]">
           <div className="flex items-center gap-4">
              ...
              <div>
                <h3 className="text-xs font-black text-[#27AE60] uppercase tracking-[0.3em] mb-1">{testData.title}</h3>
                <p className="text-[10px] font-bold text-[#5D6D61] uppercase tracking-widest">{currentSection?.section_name}</p>
              </div>
           </div>
           ...
           <div className="flex items-center gap-6">
              <div className="flex items-center gap-2 bg-green-50 px-4 py-2 rounded-xl border border-[#27AE60]/10">
                ...
                <span className={`text-sm font-mono font-black ${timeLeft < 300 ? 'text-rose-500' : 'text-[#2D3748]'}`}>{formatTime(timeLeft)}</span>
              </div>
              <div className="hidden md:flex items-center gap-2 bg-green-50 px-4 py-2 rounded-xl border border-[#27AE60]/10">
                ...
                <span className="text-sm font-black text-[#2D3748] italic">{score} XP</span>
              </div>
              {streak > 2 && (
        ...
        <div className="space-y-2">
           <div className="flex justify-between px-2">
              <span className="text-[10px] font-black text-[#2ECC71] uppercase tracking-widest">Progress</span>
              <span className="text-[10px] font-black text-[#27AE60] uppercase tracking-widest">{currentQuestionIdx + 1} / {flatQuestions.length}</span>
           </div>
                  ...
                  {currentQuestion?.type === 'listening_spelling' ? (
                    <div className="flex flex-col items-center gap-8">
                       ...
                       >
                         ...
                         <span className="font-black uppercase text-xs tracking-widest">Listen to Word (US Female)</span>
                       </button>
                    ...
                    <>
                      ...
                      {currentQuestion?.root_word && (
                        <div className="bg-[#27AE60]/5 w-fit mx-auto px-6 py-1 rounded-full border border-[#27AE60]/10 mb-2">
                          <span className="text-[10px] font-black text-[#27AE60] uppercase tracking-[0.3em]">Root Word: {currentQuestion.root_word}</span>
                        </div>
                      )}
                      <h2 className="text-3xl md:text-5xl font-black text-[#2D3748] leading-tight italic tracking-tighter max-w-5xl mx-auto">
                  ...
                  {currentQuestion?.type === 'listening_spelling' ? (
                    <div className="flex flex-col md:flex-row gap-4">
                       <input 
                         ...
                         className="flex-1 bg-green-50 border-2 border-[#27AE60]/20 rounded-[2rem] px-8 py-5 text-xl font-bold text-[#2D3748] outline-none focus:border-[#27AE60]/50 transition-all shadow-inner placeholder:text-slate-400"
                         autoFocus
                       />
                       <button 
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                       {currentQuestion?.options?.map((opt, i) => {
                         ...
                         return (
                           ...
                           >
                             <div className={`w-16 h-16 rounded-2xl flex items-center justify-center text-xl font-black border-2 transition-all shrink-0 ${isAnswered && isCorrect ? 'bg-[#27AE60] border-[#2ECC71] text-white' : 'bg-white border-[#27AE60]/10 text-[#27AE60]'}`}>
                               {String.fromCharCode(65 + i)}
                             </div>
                             <span className="font-bold text-3xl flex-1 tracking-tight">{opt}</span>
                           </button>
                         );
                       })}
                    </div>
                ...
                {isAnswered && currentQuestion && (
                  <div className="animate-slideUp space-y-8 pt-8">
                    <div className="bg-green-50 border border-[#27AE60]/20 p-8 rounded-[3rem] shadow-inner relative group">
                       <div className="flex items-start gap-5">
                          ...
                          <div className="space-y-2 flex-1">
                             <p className="text-[10px] font-black text-[#5D6D61] uppercase tracking-[0.3em]">Analysis</p>
                             <p className="text-[#2D3748] font-medium italic text-lg leading-relaxed">"{currentQuestion.explanation}"</p>
                          </div>
                       </div>
                    </div>
                    ...
                    <button 
                      ...
                      className="w-full bg-[#27AE60] text-white py-6 rounded-[3rem] font-black uppercase text-xs tracking-[0.4em] shadow-2xl hover:scale-[1.01] transition-all flex items-center justify-center gap-4 active:scale-95 border-b-8 border-[#2ECC71]"
                    >
        ...
        <footer className="py-8 text-center border-t border-slate-200">
           <div className="flex justify-center items-center gap-10 opacity-60 text-[#27AE60]">
              <div className="flex items-center gap-2"><Zap size={14} /><span className="text-[10px] font-black uppercase">Low Latency AI</span></div>
              <div className="flex items-center gap-2"><ShieldCheck size={14} /><span className="text-[10px] font-black uppercase">Anti-Hijack Voice Engine</span></div>
              <div className="flex items-center gap-2"><Target size={14} /><span className="text-[10px] font-black uppercase">Academic US Standard</span></div>
           </div>
        </footer>
      </div>
    </div>
