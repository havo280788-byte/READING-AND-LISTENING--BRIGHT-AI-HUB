    ...
    return (
      <div className="animate-fadeIn space-y-10 pb-20 max-w-2xl mx-auto flex flex-col items-center justify-center min-h-[60vh]">
        <div className="bg-white rounded-[3rem] border-4 border-[#27AE60]/20 p-12 text-center shadow-2xl w-full backdrop-blur-md">
          ...
          <h2 className="text-3xl font-black text-[#2D3748] uppercase italic mb-4">module reserved</h2>
          <button onClick={onReturn} className="w-full bg-[#27AE60] text-white py-6 rounded-[2rem] font-black uppercase text-xs tracking-[0.4em] shadow-xl hover:bg-black transition-all flex items-center justify-center space-x-3"><ArrowLeft size={18} /><span>return to hub</span></button>
        </div>
      </div>
    );
  }
    ...
    <div className="animate-fadeIn w-full flex flex-col gap-8 pb-24">
      ...
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white backdrop-blur-md px-6 py-4 rounded-[2rem] border border-green-100 shadow-xl">
        <div className="flex items-center space-x-5">
           ...
           <div><p className="text-[9px] font-black text-[#5D6D61] uppercase tracking-[0.4em] mb-0.5">{readingData.unit_context}</p><h2 className="text-lg font-black text-[#2D3748] tracking-tight">{readingData.title}</h2></div>
        </div>
        <div className="flex items-center gap-3">
           <div className="text-center px-5 py-1.5 bg-green-50 rounded-xl border border-[#27AE60]/20 shadow-sm"><p className="text-[8px] font-black text-[#5D6D61] uppercase tracking-widest">Weighted Score</p><p className="text-base font-black text-[#27AE60] italic">{weightedScore}/100</p></div>
      ...
      <div className="flex flex-col gap-8 w-full">
        ...
        <section className="w-full bg-white backdrop-blur-md rounded-[2.5rem] border-2 border-green-100 shadow-2xl relative overflow-hidden">
          ...
          <div className="p-8 md:p-12 space-y-8">
             <div className="space-y-2 border-b border-[#27AE60]/10 pb-4">
               <p className="text-[10px] font-black text-[#2ECC71] uppercase tracking-[0.4em]">Reading Passage</p>
               <h3 className="text-3xl font-black text-[#2D3748] leading-tight italic tracking-tighter">{readingData.reading_text.title}</h3>
             </div>
             <div className="prose prose-slate max-w-none">
               <p className="text-lg md:text-xl leading-[2.4] text-[#2D3748] font-medium first-letter:text-6xl first-letter:font-black first-letter:mr-4 first-letter:float-left first-letter:text-[#27AE60] whitespace-pre-wrap text-justify">
        ...
        <div className="w-full space-y-6">
          <div className="bg-white backdrop-blur-md py-3 rounded-2xl text-center border border-green-100 shadow-sm">
             <h4 className="text-[10px] font-black text-[#5D6D61] uppercase tracking-[0.6em]">Comprehension Check (Total: 100 Pts)</h4>
          </div>
                 ...
                 >
                    <div className="flex items-center gap-4">
                      ...
                      <div className="text-left">
                         ...
                         <p className="text-[8px] font-bold text-[#5D6D61] uppercase tracking-widest">{task.questions.length} QUESTIONS • {taskPoints} POINTS</p>
                      </div>
                    </div>
                    ...
                    <div className="px-6 pb-8 pt-6 bg-green-50 border-t border-[#27AE60]/10 space-y-8">
                          ...
                          <div key={q.id} className="space-y-4 p-6 bg-white rounded-3xl border border-[#27AE60]/10 shadow-sm">
                             ...
                             <div className="flex gap-4 pl-8 pt-2">
                                {['True', 'False'].map((label) => {
                                  ...
                                  return (<button key={label} disabled={isAnswered[q.id]} onClick={() => handleTFAnswer(q.id, val, task)} className={`flex-1 py-4 rounded-2xl font-bold text-lg uppercase tracking-wide border-2 transition-all ${btnClass}`}>{label}</button>);
                                })}
                             </div>
                             {isAnswered[q.id] && <p className="pl-8 text-sm font-medium italic text-[#5D6D61] animate-fadeIn mt-2 bg-slate-50 p-3 rounded-xl border border-slate-100">Note: {q.explanation}</p>}
                          </div>
                       ))}
                       {task.type === 'fill_in_blanks' && task.questions.map(q => (
                          <div key={q.id} className="space-y-4 p-6 bg-white rounded-3xl border border-[#27AE60]/10 shadow-sm">
                             <p className="text-xl font-medium text-[#2D3748] leading-relaxed">
                               <span className="text-[#27AE60] font-black mr-3 text-xl">{q.id}.</span>
                               {q.sentence?.split('_______').map((part, i) => (
                                 <React.Fragment key={i}>{part}{i === 0 && (<span className="inline-block border-b-4 border-[#27AE60] min-w-[100px] mx-2 text-center text-[#27AE60] font-bold px-2 text-xl">{isAnswered[q.id] ? (userAnswers[q.id] as string) : '_______'}</span>)}</React.Fragment>
                               ))}
                             </p>
                             {!isAnswered[q.id] ? (
                               <div className="flex gap-3 pl-8">
                                  <input type="text" placeholder={`Hint: ${q.hint}`} className="bg-slate-50 border-2 border-[#27AE60]/30 rounded-2xl px-5 py-3 text-lg font-medium text-[#2D3748] outline-none focus:border-[#27AE60] w-full shadow-inner" onKeyDown={(e) => { if (e.key === 'Enter') handleGapFill(q.id, (e.target as HTMLInputElement).value, task); }} />
                                  <button onClick={(e) => { const input = (e.currentTarget.previousSibling as HTMLInputElement); handleGapFill(q.id, input.value, task); }} className="bg-[#27AE60] text-white px-6 py-3 rounded-2xl font-bold text-sm uppercase hover:bg-[#2ECC71] transition-all shadow-md active:scale-95">OK</button>
                               </div>
                             ) : (
                               <div className="pl-8 animate-fadeIn mt-2"><p className="text-sm font-medium italic text-[#5D6D61] bg-slate-50 p-3 rounded-xl border border-slate-100"><span className={`font-bold mr-2 ${userAnswers[q.id]?.toString().toLowerCase() === q.answer.toString().toLowerCase() ? 'text-[#27AE60]' : 'text-[#FF0000]'}`}>Answer: {q.answer as string}</span> • {q.explanation}</p></div>
                             )}
                          </div>
                       ))}
                       {task.type === 'multiple_choice' && task.questions.map(q => (
                          <div key={q.id} className="space-y-5 p-6 bg-white rounded-3xl border border-[#27AE60]/10 shadow-sm">
                             <p className="text-xl font-medium text-[#2D3748] leading-relaxed"><span className="text-[#27AE60] font-black mr-3 text-xl">{q.id}.</span> {q.question}</p>
                                ...
                                {q.options?.map((opt, i) => {
                                  ...
                                  return (
                                    <button 
                                      ...
                                      className={`p-4 rounded-xl text-left font-medium text-lg border-2 transition-all flex items-center gap-4 shadow-sm ${btnClass}`}
                                    >
                                      <span className="w-8 h-8 rounded-lg bg-white/50 flex items-center justify-center text-sm font-black border border-black/5 shrink-0">
                             ...
                             {isAnswered[q.id] && <p className="pl-8 text-sm font-medium italic text-[#5D6D61] animate-fadeIn mt-2 bg-slate-50 p-3 rounded-xl border border-slate-100">{q.explanation}</p>}
                          </div>
                       ))}

                       {isTaskAnswered && idx < readingData.tasks.length - 1 && (
                         <button onClick={() => { setOpenTaskIndex(idx + 1); playPop(); }} className="w-full py-4 mt-6 rounded-2xl bg-green-50 text-[#27AE60] font-black uppercase text-xs tracking-[0.2em] border border-[#27AE60]/30 hover:bg-green-100 transition-all flex items-center justify-center gap-2">NEXT TASK <ChevronDown size={14} /></button>
                       )}
                    </div>
                 </div>
              </div>
